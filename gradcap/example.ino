...
SMARTMATRIX_ALLOCATE_SCROLLING_LAYER(anotherLayer, kMatrixWidth, kMatrixHeight, COLOR_DEPTH, kScrollingLayerOptions);
...
void setup() {
 ...
 // delay(3000);
  ...
  scrollingLayer.setOffsetFromTop((kMatrixHeight/2) - 15);
  scrollingLayer.setStartOffsetFromLeft(4);


  anotherLayer.setMode(stopped);
  anotherLayer.setColor({0xff, 0xff, 0xff});
  anotherLayer.setSpeed(15);
  anotherLayer.setFont(font8x13);
  anotherLayer.start("ECE", -1);
  anotherLayer.setOffsetFromTop((kMatrixHeight/2)+0 );
  anotherLayer.setStartOffsetFromLeft(4);
}
